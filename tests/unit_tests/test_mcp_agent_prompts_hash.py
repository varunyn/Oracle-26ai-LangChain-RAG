...
def test_system_prompt_mixed_hash_locked():
    ...
    actual_hash = hashlib.sha256(SYSTEM_PROMPT_MIXED.encode("utf-8")).hexdigest()
...
def test_mixed_prompt_disallows_general_knowledge_after_empty_oracle_retrieval():
    assert "do not answer from general knowledge" in SYSTEM_PROMPT_MIXED
    assert "selected Oracle collection does not contain the answer" in SYSTEM_PROMPT_MIXED
