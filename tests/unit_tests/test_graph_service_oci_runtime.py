...
def test_graph_service_run_chat_defaults_to_mixed_when_mcp_enabled(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "src.rag_agent.runtime.mcp_turn.load_adapter_tools", fake_load_adapter_tools
    )
    monkeypatch.setattr(
        "src.rag_agent.runtime.mcp_turn.get_mcp_answer_async", fake_get_mcp_answer_async
    )
    monkeypatch.setattr(
        ChatRuntimeService,
...
def test_graph_service_run_chat_mcp_mode_uses_mcp_answer_async(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "src.rag_agent.runtime.mcp_turn.get_mcp_answer_async",
...
def test_graph_service_run_chat_mixed_mode_uses_mcp_answer_async(monkeypatch) -> None:
    ...
    async def fake_get_mcp_answer_async(
        ...
        **kwargs: object,
        ...
        captured["tools"] = tools
        captured["run_config"] = run_config
        captured["stop_after_tool_names"] = kwargs.get("stop_after_tool_names")
        return ("The integral is (x^2 - 2x + 2)e^x + C.", ["calculator_tool"], [])
    ...
    assert captured["tool_server_keys"] == ["calculator"]
    assert captured["require_tool_call"] is False
    assert captured["stop_after_tool_names"] is None
    assert result["mcp_tools_used"] == ["calculator_tool"]
...
def test_graph_service_mixed_mode_lets_agent_loop_continue_with_retrieval_tool(
    ...
    async def fake_get_mcp_answer_async(
        *args: object, **kwargs: object
        ...
        _ = args
        captured["stop_after_tool_names"] = kwargs.get("stop_after_tool_names")
        return ("Retrieved answer.", ["retrieval_tool"], [])
    ...
    result = asyncio.run(
        service.run_chat(
            ...
            model_id="google.gemini-2.5-pro",
            thread_id="thread-mixed-rag-only",
            session_id=None,
            collection_name="RAG_KNOWLEDGE_BASE",
            enable_reranker=None,
            enable_tracing=None,
...
def test_graph_service_mixed_mode_runs_repeated_workflow_before_normal_agent(
    ...
    monkeypatch.setattr(
        "src.rag_agent.runtime.mcp_turn.get_mcp_answer_async", fail_if_normal_agent_runs
    )
    monkeypatch.setattr(
        "src.rag_agent.runtime.chat_service.get_settings",
        lambda: type(
    ...
    monkeypatch.setattr(
        "src.rag_agent.runtime.mcp_turn.should_use_repeated_workflow",
        fake_should_use_repeated_workflow,
    )
    monkeypatch.setattr(
        "src.rag_agent.runtime.mcp_turn.run_repeated_mcp_workflow",
