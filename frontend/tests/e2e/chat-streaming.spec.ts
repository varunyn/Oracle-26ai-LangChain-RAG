...
test.describe("chat streaming", () => {
  test("shows generic suggestions on first load", async ({ page }) => {
    ...
    await expect(
      suggestions.getByRole("button", {
        name: "Tell me about Oracle 26ai Database.",
      })
    ).toBeVisible();
    await expect(
      suggestions.getByRole("button", {
        name: "Solve this math problem: 125 * 48.",
      })
    ).toBeVisible();
    await expect(
      suggestions.getByRole("button", {
        name: "What can you help me find in my documents?",
      })
    ).toBeVisible();
    await expect(
  ...
  test("does not render clicked suggestions as duplicate user messages", async ({
    page,
  }) => {
    ...
    await expect(
      messageList.getByText("Oracle 26ai deployment details.")
    ).toBeVisible();
    await expect(
      messageList.getByText(suggestion, { exact: true })
    ).toHaveCount(1);
  });
  ...
  test("shows server-backed chat history and switches active threads", async ({
    page,
  }) => {
  ...
  test("keeps long chat history scrollable in the sidebar", async ({
    ...
    const initialLastThreadPosition = await history.evaluate((historyNode) => {
      ...
      if (!button) {
        throw new Error("Expected final history item");
      }
  ...
  test("keeps chat history title updates stable after asking", async ({
    page,
  }) => {
        ...
        body: JSON.stringify([
          {
            ...
            updated_at: "2026-06-26T10:00:00Z",
            values: {
              messages: [{ type: "human", content: "Existing title" }],
            },
          },
        ]),
      });
    ...
    await mockLangGraphProtocol(page, [
      {
        method: "values",
        data: {
          messages: [{ type: "human", content: "Will title updates loop?" }],
        },
      },
      {
        method: "values",
    ...
    await page
      ...
      .fill("Will title updates loop?");
    await page.getByRole("button", { name: "Ask" }).click();

    await expect(page.getByText("No.")).toBeVisible();
    await expect(page.getByText("This page couldn’t load")).toHaveCount(0);
    expect(
      pageErrors.filter((message) => message.includes("Maximum update depth"))
    ).toEqual([]);
  });
  ...
  test("renders native tool calls from stream messages and tool events", async ({
    page,
  }) => {
    const prompt =
    ...
    await expect(
      page.getByText("The best-fit line is y = 1.54x + 0.44.")
    ).toBeVisible();
    await expect(
      page.locator(
  ...
  test("renders native tool calls once the final assistant payload arrives", async ({
      ...
      ({ promptText, answerText }) => {
        ...
        window.fetch = (input: RequestInfo | URL, init?: RequestInit) => {
          ...
          if (url.includes("/threads/search")) {
            historyCalls += 1;
            (
              window as typeof window & { __historyCalls?: number }
          ...
          if (url.includes("/threads/") && url.endsWith("/stream/events")) {
            ...
            const channels = Array.isArray(streamBody.channels)
              ? streamBody.channels
              : [];
            if (
              !(channels.includes("values") || channels.includes("messages"))
            ) {
              return Promise.resolve(
                new Response("", {
                  status: 200,
            ...
            (
              window as typeof window & { __releaseChatFinal?: () => void }
            ).__releaseChatFinal = () => releaseFinal?.();
    ...
    await expect(
      page.locator(
        '[data-tool-type="tool-oracle_retrieval"][data-tool-state="output-available"]'
      )
    ).toBeVisible();
  ...
  test("keeps native tool cards matched to the correct assistant message across lifecycle updates and replay", async ({
    ...
    const firstAnswer =
      "I checked the invoice totals and requested the vendor lookup.";
    const secondAnswer =
    ...
    await expect(
      firstAssistant.locator(
        '[data-tool-type="tool-lookup_vendor_profile"][data-tool-state="output-error"]'
      )
    ).toBeVisible();
    await expect(
      secondAssistant.locator(
        '[data-tool-type="tool-summarize_invoice_risk"][data-tool-state="output-available"]'
      )
    ).toBeVisible();
  ...
  test("expands the chat input for long multi-line prompts", async ({
    ...
    const longPrompt = Array.from(
      { length: 24 },
      (_, index) =>
        `"InvoiceLine${index + 1}": { "Description": "Product ${index + 1}" }`
    ).join("\n");
  ...
  test("keeps API connection failures out of the browser error overlay", async ({
    ...
    await page
      .getByRole("textbox", { name: "Message" })
      .fill("Will this fail gracefully?");
    await page.getByRole("button", { name: "Ask" }).click();
