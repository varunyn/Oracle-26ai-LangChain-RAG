...
def build_records() -> list[dict[str, Any]]:
    ...
    for thread_id, (checkpoint, meta) in latest.items():
        ...
        user_request = str(
            ...
            or next((m["content"] for m in turns if m["role"] == "Human"), "")
        )
        ...
        records.append(
            {
                ...
                "label": (
                    "error-candidate"
                    if not answer.strip() or answer.strip() == user_request.strip()
                    else "unreviewed"
                ),
                "mode": mode,
                ...
                "features": [
                    ...
                    chars,
                    len(citations),
                    len(state.get("retriever_docs") or []),
                    len(state.get("reranker_docs") or []),
                    {"direct": 0, "mixed": 1, "rag": 2, "mcp": 3, "unknown": 4}.get(mode, 4),
                ],
            }
        )
...
def prepare() -> None:
    ...
    for i, r in enumerate(records):
        ...
        graph.append(
            {
                "id": r["id"],
                "x": (f[1] / 900 + f[2] * 0.7) % 10,
                "y": (f[0] * 1.5 + f[3] * 0.4 + f[5] * 1.2) % 10,
                "cluster": (f[5] + f[0]) % 7,
                "sample": r in samples,
            }
        )
