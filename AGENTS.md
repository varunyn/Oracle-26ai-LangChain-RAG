...
- For agentic AI, MCP, tool-calling, routing, retry, memory/state, streaming, or output-quality bugs, inspect live Langfuse v4 observations before coding. Use `langfuse --env .env api observations list --limit 5 --fields basic,metadata,model,usage,metrics --json`, then use `--trace-id <trace-id>` with the same command to inspect one hierarchy.
...
- For agentic AI, tool-calling, MCP, routing, or LLM behavior debugging, inspect live observations with the installed Langfuse CLI before guessing from code. Prefer `langfuse --env .env api observations list --limit 5 --fields basic,metadata,model,usage,metrics --json`, then repeat it with `--trace-id <trace-id>` for one hierarchy. The local self-hosted stack is Langfuse v4; do not use the deprecated `api traces list` endpoint. Never print public/secret key values.
