...
The `/api/langgraph/threads/{thread_id}/commands` flow keeps thread-scoped memory in `ChatRuntimeService` for the current API process. This does not change tracing/observability wiring, but you may see prior-turn context reflected in LangChain spans and Langfuse metadata (for example `mcp_used`, `mcp_tools_used`, `standalone_question`, and `context_usage`).
