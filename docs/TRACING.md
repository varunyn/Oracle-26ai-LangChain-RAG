...
Tracing is done **via the LangChain callback stack** (no API-level manual trace). When enabled, each `/api/langgraph/threads/{thread_id}/runs/stream` request produces one Langfuse trace with nested spans for runtime steps, LLM calls, and tools.
