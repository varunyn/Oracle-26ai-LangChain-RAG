...
5. Inspect the trace in Langfuse (Sessions → latest trace). For CLI investigations on the v4 stack, use `langfuse --env .env api observations list --limit 50 --fields basic,metadata,model,usage,metrics --json`; the old `traces list` endpoint is deprecated. The SDK runs fail-open—if Langfuse is offline, requests continue without blocking.
