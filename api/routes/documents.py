    ...
    try:
        for f in files:
            ...
            if ext not in allowed:
                continue
            path = Path(temp_dir) / (f.filename or "upload")
            content = await f.read()
            path.write_bytes(content)
            paths.append(path)
        if not paths:
            return {"error": "No supported files (pdf, html, htm, txt, md)", "chunks_added": 0}
        ...
        success, num_chunks, err = await asyncio.to_thread(
            process_file_paths, cast(list[str | Path], paths), table_name
        )
        if success:
            return {
        ...
        for p in paths:
            try:
                ...
                logger.debug("Cleanup unlink %s: %s", p, e)
        try:
            Path(temp_dir).rmdir()
        except OSError as e:
            logger.debug("Cleanup rmdir %s: %s", temp_dir, e)
...
@router.get("/sources")
async def list_document_sources(collection_name: str | None = Query(default=None)):
...
async def delete_document_source(
    ...
    deleted_chunks = await asyncio.to_thread(
        delete_source_from_collection, table_name, source.strip()
    )
    return {
        "collection": table_name,
        "source": source.strip(),
